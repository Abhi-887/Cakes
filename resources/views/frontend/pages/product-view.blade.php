        ...
        $(document).ready(function() {
            ...
            function v_updateTotalPrice() {
                ...
                $('select[name="variants_items[]"], input[name="variants_items[]"]:checked, input[name="variants_items[][]"]:checked')
                    .each(function() {
                        let price = parseFloat($(this).find('option:selected').text().match(/\$(\d+(\.\d+)?)/)
                            ?.[1] || $(this).next('label').text().match(/\$(\d+(\.\d+)?)/)?.[1] || 0);
            ...
            $('select[name="variants_items[]"], input[name="variants_items[]"], input[name="variants_items[][]"]')
                .on('change', function() {
                    v_updateTotalPrice();
                });
